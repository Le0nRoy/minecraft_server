...
.NOTES
    This file is both a valid .bat and a valid PowerShell script - just
    double-click it in Explorer. Under the hood it re-invokes itself via
    `powershell -NoExit -ExecutionPolicy Bypass -File "%~f0"`, which is
    exactly what you'd type by hand if running it from a PowerShell prompt
    instead:
        powershell -NoExit -ExecutionPolicy Bypass -File install-client-windows.bat
#>
